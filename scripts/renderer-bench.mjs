...
async function run(mode) {
  ...
  await page.goto(`http://localhost:${PORT}/?bench=${mode}&ropes=${ROPES}`, { waitUntil: "networkidle" });
  // Skia has to fetch and compile ~9MB of wasm before it draws anything.
  await page.waitForTimeout(mode === "skia" ? 9000 : 3000);

  const drew = await page.evaluate(() =>
    document.querySelectorAll("canvas").length > 0 || document.querySelectorAll("svg path").length > 0,
  );
  if (!drew) {
    console.log(`${mode.padEnd(5)} DID NOT RENDER — ${errors.slice(0, 2).join(" | ") || "no error reported"}`);
    await ctx.close();
    return;
  }
  ...
  console.log(
    `${mode.padEnd(5)} fps ${(frames.length / wallS).toFixed(1).padStart(5)}  ` +
...
console.log(`\n${ROPES} ropes, three strokes each, 8s steady state\n`);
await run("svg");
await run("skia");
await browser.close();
server.close();
